        ...
        <div class="col-xl-12">
          ...
          <!--   <div class="widget has-shadow">
            ...
            <DataTableVue4
                    ...
                    :rowData="businessInvoice"
                    :btnList="[
                            {btnName: '엑셀다운로드', onClick: () => { this.openModalPopup('') }},
                            {btnName: '+ 신규등록', onClick: () => { this.openModalPopup('salesSpec') }}
                          ]"
                    :isPaging="true"
                    :rowEditorList="[{
                        type: 'BUTTON',
                        editColumn: 'button',
                        label: '수정',
                        onClick: (data) => { this.openModalPopup('salesSpec', {index: data.row.counselNo, id: data.row.userIdCreatedBy}) }
                        },
                        {
                        type: 'BUTTON1',
                        editColumn: 'button',
...
export default {
  ...
  components: { VendorListPopup, SalesSpecPopup, DataTableVue4 },
  data () {
    return {
      modalOpenFlag: false,
      businessInvoice: [],
  ...
  created () {
    this.getBusinessInvoice()
  },
  computed: {
    ...mapGetters(salesStore, [
      'getColumnDef'
    ])
  },
  methods: {
    ...mapActions(salesStore, [
      'actionFindBusinessInvoicelList'
    ]),
    getBusinessInvoice () {
      this.actionFindBusinessInvoicelList()
        .then(res => {
          console.log(res.data)
          this.businessInvoice = res.data
        })
        .catch(err => {
          console.log(err)
        })
