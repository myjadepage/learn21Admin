...
export default {
  ...
  created () {
    ...
    this.actionFindCategoryCode().then(res => {
      this.actionSetBigCodeList(this.lodash.cloneDeep(res.filter(x => x.icParentNo === null))) // 대분류코드
    })
  },
  data () {
    return {
  ...
  methods: {
    ...
    onClickAddBigCode () {
      ...
      addList.push({
        icCodeNo: 'add_' + this.getBigCodeList.length,
        icCodeName: this.addBigCodeName,
        icParentNo: null,
        icOrderby: this.getBigCodeList.length + 1,
        icUseYn: 'Y'
      })
    ...
    onClickAddMidCode () {
      ...
      addList.push({
        ...
        icParentNo: document.querySelector("[name='rdoBigCode']:checked").value,
