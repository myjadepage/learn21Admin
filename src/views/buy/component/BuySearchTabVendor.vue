                                  ...
                                  <template v-for="dataList in byGroupRow">
                                    <template v-for="(data, i) in dataList">
                                      <tr :key="data.index">
                                          ...
                                          <td class="text-left" :class="(data.itemName === '[소계]') ? 'text-center table-total-color' : ''">{{ data.itemName }}</td>
...
export default {
  ...
  methods: {
    getRowspan (list) {
      return list.reduce((acc, curr, i, array) => {
        return i
      })
    },
    aaa (list) {
      let aa = list.reduce((acc, el) => ({...acc, [el.diiPurchdate]: (acc[el.diiPurchdate] || 0) + 1}), {})
      return Object.values(aa).map((el, index) => {
